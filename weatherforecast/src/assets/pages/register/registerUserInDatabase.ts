...
const registerUserInDatabase = async (data: FieldValues, existingUsers: UserType[]) => {
    ...
    const newUser: UserType = { id: id, picture: '/icons/overig/user.svg', email: data.email, firstName: data.firstName, lastName: data.lastName, password: data.password, temperature: 'Celcius', mode: 'Darkmode', cityOne: 'Maastricht', cityTwo: '\'s-Hertogenbosch', cityThree: 'Middelburg' }

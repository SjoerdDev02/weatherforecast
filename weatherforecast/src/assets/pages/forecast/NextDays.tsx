...
const NextDay = ({ day, weather, minTemp, maxTemp }: { day: number, weather: number, minTemp: number, maxTemp: number }) => {
    ...
    const daysArr = ['Mon', 'Tue', 'Wen', 'Thu', 'Fri', 'Sat', 'Sun', 'Mon', 'Tue', 'Wen', 'Thu'];
...
const NextDays: React.FC<NextDaysProps> = ({ time, dayWeather, dayMinTemp, dayMaxTemp }) => {
    const currentDayIndex = new Date(time).getDay();
    const nextDaysElements: React.JSX.Element[] = [];
    
    const createNextDaysElements = () => {
        for (let i = 0; i < 4; i++) {
            nextDaysElements.push(<NextDay key={i} day={currentDayIndex + i} weather={dayWeather[i]} minTemp={dayMinTemp[i]} maxTemp={dayMaxTemp[i]} />)
        }
    }

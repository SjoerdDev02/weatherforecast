...
const TodayHeader = ({ location, temperature, humidity }: { 
    location: string; 
    temperature: number; 
    humidity: number;
}) => {
...
const TodayNumbers = ({
    ...
    const weatherDescription = weatherCodes.find((weatherCode) => weatherCode.code.includes(weather))!.description;
...
const HourNumbers = ({ hour, weather, temperature }: { hour: string, weather: number; temperature: number; }) => {
  const weatherIcon = '/icons/weather/sun-cloud-dark.svg';
...
const HoursContainer = ({hour, hourWeather, hourTemperature}: { hour: number, hourWeather: number[], hourTemperature: number[] }) => {
    const hoursElements: React.JSX.Element[] = [];
    
    const createHoursElements = () => {
        for (let i = 0; i < 8; i++) {
            const currentHour = hour + i < 12 ? `${hour + i} AM` : `${hour + i - 12} PM`;
            hoursElements.push(<HourNumbers key={hour + i} hour={currentHour} weather={hourWeather[i]} temperature={hourTemperature[i]} />)
        }
    }
...
const TodayWeather: React.FC<TodayWeatherProps> = ({
    ...
    return (
        ...
        <TodayHeader location={location} temperature={temperature} humidity={humidity} />
        <TodayNumbers weather={weather} rainProb={rainProb[hour + 1]} appTemp={appTemp} visibility={visibility[hour + 1]} />
        <HoursContainer hour={hour} hourWeather={selectedHoursWeather} hourTemperature={selectedHoursTemperature} />
        </section>
    );
};
